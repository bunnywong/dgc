	...
	class Better_User_Search {
		...
		public function pre_user_query( $wp_user_query ) {
			...
			if ( strpos( $wp_user_query->query_where, '@' ) === false
				...
				// Empty the table in case we didn't have permissions to drop it
				$wpdb->query( $sql = "TRUNCATE TABLE {$mktable};" );

				// Insert the meta keys into our table
				foreach ( $meta_keys as $meta_key ) {
					$wpdb->insert( $mktable, array( 'meta_key' => $meta_key ) );
				}
				...
				$wpdb->query( $sql = "DROP TABLE IF EXISTS temp_better_user_search_meta_keys;" );
